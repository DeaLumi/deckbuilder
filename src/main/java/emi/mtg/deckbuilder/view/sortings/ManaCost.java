...
public class ManaCost implements CardView.Sorting {
	...
	public int compare(CardInstance c1, CardInstance c2) {
		...
		for (int i = emi.lib.mtg.characteristic.Color.values().length - 1; i >= 0; --i) {
			...
			long n1 = -c1.card().manaCost().symbols().stream().filter(Objects::nonNull).map(ManaSymbol::color).filter(s -> s != null && s.contains(c)).count();
			long n2 = -c2.card().manaCost().symbols().stream().filter(Objects::nonNull).map(ManaSymbol::color).filter(s -> s != null && s.contains(c)).count();
