 ...
 */
public class NewPilesView extends HBox {
	private final FilteredList<CardInstance> filteredCards;
	private final SortedMap<String, CardGroup> groupMap;

	public NewPilesView(ObservableList<CardInstance> cards, Comparator<String> groupSort, Function<CardInstance, String> group, Comparator<CardInstance> sort, ImageSource images, Gson gson) {
		...
		for (CardInstance ci : cards) {
			...
			this.groupMap.computeIfAbsent(groupName, g -> new CardGroup(g, gson, images, filteredCards, sort, "Piles", TransferMode.ANY, new TransferMode[] { TransferMode.MOVE }));
		}
